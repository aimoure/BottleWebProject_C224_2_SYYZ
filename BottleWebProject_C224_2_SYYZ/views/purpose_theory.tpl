...
<div class="container">
    ...
    <img src="/static/images/Condition.png" alt="Условие" class="img-fluid">
   ...
   <img src="/static/images/Step2.png" alt="Редукция по столбцам" class="img-fluid">
    ...
    <p>Cмотрим чтобы в каждом столбце и в каждой строке был только один выбранный ноль. Как видно ниже, в данном случае это сделать невозможно. Два варианта как можно это сделать:</p>
    <img src="/static/images/Step3_1.png" alt="Нули" class="img-fluid">
    <p>Продолжаем решение дальше. Вычеркиваем строки и столбцы, которые содержат нулевые элементы (ВАЖНО! 
    ...
    пересечении вычеркнутых строк и столбцов (то, что отмечено зеленым – там вычитаем; то, что отмечено золотистым – 
    там суммируем; то, что не закрашено – не трогаем):</p>
    ...
    <p>Подставляем в начальную таблицу месторасположения выбранных нулей. Таким образом мы получаем оптимум, или оптимальный план, при котором 
    организаторы распределены по работам и сумма оценок получилась максимальной:</p>
    <img src="/static/images/Step5.png" alt="Нули" class="img-fluid">
    <p>Если же вы решаете задачу и у вас до сих пор невозможно выбрать нули так, чтобы в каждом столбце и строке был только один, тогда повторяем 
    алгоритм с того места где проводилась редукция по строкам (минимальный элемент в каждой строке).</p>
